...
for record in all_data:
    record_0 = ','.join(record[:2])
    record_2 = ','.join(record[2:])
    repeated_dict[record_0] = repeated_dict.setdefault(record_0, 0) + 1
    if repeated_dict.setdefault(record_0) != 1:
        repeated_data.append(record_0)
for record in all_data:
    record_0 = ','.join(record[:2])
    record_2 = ','.join(record[2:])
    # print(record_0,record_2)
    data_dct = {record_0: record_2}
...
sorted_str_1 = ','.join(v_sorted_list_1)
for key, value in value_dict.items():
    if key in repeated_data[0]:
        value_dict[key] = sorted_str_1
...
sorted_str_2 = ','.join(v_sorted_list_2)
for key, value in value_dict.items():
    if key in repeated_data[1]:
        value_dict[key] = sorted_str_2
